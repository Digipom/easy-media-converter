...
public class CutActionFragment extends Fragment implements MainButtonInterfaces.HandleMainButtonTapListener {
    public interface OnCutActionFragmentInteractionListener {
        void onCutSelected(@NonNull Uri targetUri,
    ...
    private static final int UPDATE_CUT_START_TIME_REQUEST_CODE = 2;
    private static final int UPDATE_CUT_END_TIME_REQUEST_CODE = 3;

    @NonNull
    public static CutActionFragment create() {
        return new CutActionFragment();
    }

    @Override
    public View onCreateView(@NonNull LayoutInflater inflater, @Nullable ViewGroup container,
                             @Nullable Bundle savedInstanceState) {
        return inflater.inflate(R.layout.fragment_cut_action, container, false);
    }

    private CutActionViewModel viewModel;
    ...
    public void onActivityCreated(@Nullable Bundle savedInstanceState) {
        ...
        viewModel = new ViewModelProvider(this).get(CutActionViewModel.class);
        ...
        final TextView cutStartTextView = view.findViewById(R.id.cut_start_textview);
        final TextView cutEndTextView = view.findViewById(R.id.cut_end_textview);
        final String cutStartContentDescription = getString(R.string.cutStartingAtContentDescription);
        final String cutEndContentDescription = getString(R.string.cutEndingAtContentDescription);
        ...
        rangeSeekBar.setRangePinsByIndices((int) (rangeSeekBar.getTickEnd() / 2), (int) (rangeSeekBar.getTickEnd() / 2));
        ...
        viewModel.cutStartMs().observe(getViewLifecycleOwner(), new Observer<Long>() {
            @Override
            public void onChanged(@Nullable Long cutStartTimeMs) {
                formatDurationWithTenths(builder, cutStartTextView, Objects.requireNonNull(cutStartTimeMs), cutStartContentDescription);
                ...
                if (!isUserModifyingRangeSeekBar.get()) {
                    final float ratio = viewModel.getRatioForMs(cutStartTimeMs);
        ...
        viewModel.cutEndMs().observe(getViewLifecycleOwner(), new Observer<Long>() {
            @Override
            public void onChanged(@Nullable Long cutEndTimeMs) {
                formatDurationWithTenths(builder, cutEndTextView, Objects.requireNonNull(cutEndTimeMs), cutEndContentDescription);

                // Also sync the range seek bar.
                // To prevent cascades, we do a direct update of the range seekbar here.
                if (!isUserModifyingRangeSeekBar.get()) {
                    final float ratio = viewModel.getRatioForMs(cutEndTimeMs);
                    final int index = (int) (ratio * rangeSeekBar.getTickEnd());
            ...
            public void onRangeChangeListener(RangeBar rangeBar, int leftPinIndex, int rightPinIndex,
                ...
                if (leftPinIndex != previousLeftPinIndex || rightPinIndex != previousRightPinIndex) {
                    ...
                    if (leftPinIndex != previousLeftPinIndex) {
                        sharedViewModel.onActionWantsToChangePlaybackPositionToOffset(Objects.requireNonNull(viewModel.cutStartMs().getValue()));
                        previousLeftPinIndex = leftPinIndex;
                    } else if (rightPinIndex != previousRightPinIndex) {
                        sharedViewModel.onActionWantsToChangePlaybackPositionToOffset(Objects.requireNonNull(viewModel.cutEndMs().getValue()));
        ...
        cutStartTextView.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                if (getLifecycle().getCurrentState().isAtLeast(Lifecycle.State.STARTED)) {
                    final FragmentManager fragmentManager = getParentFragmentManager();
                    final long defaultValue = Objects.requireNonNull(viewModel.cutStartMs().getValue());
                    final long minValue = 0;
                    final long maxValue = Objects.requireNonNull(viewModel.cutEndMs().getValue());

                    TimePickerDialogFragment.show(fragmentManager, CutActionFragment.this,
                            UPDATE_CUT_START_TIME_REQUEST_CODE,
                            getString(R.string.cutStart),
                            defaultValue, minValue, maxValue);
                }
            }
        });
        cutEndTextView.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                if (getLifecycle().getCurrentState().isAtLeast(Lifecycle.State.STARTED)) {
                    final FragmentManager fragmentManager = getParentFragmentManager();
                    final long defaultValue = Objects.requireNonNull(viewModel.cutEndMs().getValue());
                    final long minValue = Objects.requireNonNull(viewModel.cutStartMs().getValue());
                    final long maxValue = Objects.requireNonNull(sharedViewModel.getObservableDurationMs().getValue());

                    TimePickerDialogFragment.show(fragmentManager, CutActionFragment.this,
                            UPDATE_CUT_END_TIME_REQUEST_CODE,
                            getString(R.string.cutEnd),
                            defaultValue, minValue, maxValue);
                }
            }
        });
    ...
    // TODO require cut start to be at least 0.1ms from the beginning, and same for the cut end.
            ...
            IntentUtils.handleNewlyCreatedDocumentFromActivityResult(
                    Objects.requireNonNull(activity), resultCode, data, new IntentUtils.NewDocumentHandler() {
                        ...
                        public void onReceivedUriForNewDocument(@NonNull Uri target) {
                            final String outputFilename = viewModel.getDefaultOutputFilename();
                            final long cutStartMs = Objects.requireNonNull(viewModel.cutStartMs().getValue());
                            final long cutEndMs = Objects.requireNonNull(viewModel.cutEndMs().getValue());
                            final long durationMs = Objects.requireNonNull(sharedViewModel.getObservableDurationMs().getValue());
                            ((OnCutActionFragmentInteractionListener) activity).onCutSelected(target, outputFilename, cutStartMs, cutEndMs, durationMs);
                        }
                    });
        } else if (requestCode == UPDATE_CUT_START_TIME_REQUEST_CODE) {
            if (resultCode == RESULT_OK && data != null) {
                final long ms = data.getLongExtra(INTENT_EXTRA_MS, -1);
                if (ms >= 0) {
                    viewModel.onCutStartModified(ms);
                    sharedViewModel.onActionWantsToChangePlaybackPositionToOffset(Objects.requireNonNull(viewModel.cutStartMs().getValue()));
                }
            }
        } else if (requestCode == UPDATE_CUT_END_TIME_REQUEST_CODE) {
            if (resultCode == RESULT_OK && data != null) {
                final long ms = data.getLongExtra(INTENT_EXTRA_MS, -1);
                if (ms >= 0) {
                    viewModel.onCutEndModified(ms);
                    sharedViewModel.onActionWantsToChangePlaybackPositionToOffset(Objects.requireNonNull(viewModel.cutEndMs().getValue()));
                }
            }
        }
